...
def next_word():
    ...
    #print(curr_word['English'])
    #lan_val = target_lan.get()
...
#word_frame = customtkinter.CTkFrame(root, width=400, height=300, corner_radius=10)
#word_frame.grid(row=1, column=1, columnspan=2, pady=(50,0),padx=(45,0))
...
btn_next = customtkinter.CTkButton(master=root, text='Next', hover=True, command=next_word)
...
#word_text = Text(master=word_frame, bd=0,bg='#2B2B2B', fg='silver', borderwidth=0, wrap=WORD, highlightthickness=0,state='disabled')
#word_text.pack(pady=10, padx=10)
